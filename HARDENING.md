...
- `unwrap_used`, `panic`, `unreachable`, `unwrap_in_result`, `panic_in_result_fn`, `get_unwrap`: zero non-test fallout, scoped to non-test via the crate-root `cfg_attr` in item 5. Locks in the no-unwrap, no-panic, expect-only discipline. Phase 3.
- `mem_forget`, `mutex_atomic`, `rc_buffer`, `path_buf_push_overwrite`, `verbose_file_reads`, `lossy_float_literal`, `missing_assert_message`, `self_named_module_files`: all zero fallout. Pure guardrails that cost nothing now and catch a future regression. Adopt as a single low-risk group.
...
3. The unwrap, panic, and unreachable bans via the crate-root `cfg_attr` in `rayonet/src/lib.rs`, plus the zero-fallout guardrail group as a second small commit. Gate green at each.
...
5. `[lints] workspace = true` on the five consumer-style members, fixing the group fallout.
6. Final tidy: consider `todo` and `unimplemented` to deny, plus any dead code, naming, and doc gaps. Final full gate.
