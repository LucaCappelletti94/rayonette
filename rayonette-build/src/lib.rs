//! Build-time extraction for rayonette, called from a consumer crate's `build.rs`.
//! It generates the agent task registry and bundles a buildable source tree to
//! ship to workers, both included by `rayonette::embed_microcrates!()`.
//!
...
//! a worker can resolve and build it even when rayonette is an unpublished path
//! dependency.
...
///
/// Produces `rayonette::agent::Registry::new().with_fn(a).with_fn(b)`, included
/// verbatim by `rayonette::embed_microcrates!()` in the consumer crate.
#[must_use]
pub fn generate_registry(functions: &[String]) -> String {
    use std::fmt::Write as _;
    let mut code = String::from("rayonette::agent::Registry::new()");
...
///
/// Writes the generated agent registry to `OUT_DIR/rayonette_registry.rs` and the
/// shippable source bundle to `OUT_DIR/rayonette_source.tar`, both for
...
fn extract_into(
    ...
    std::fs::write(
        out_dir.join("rayonette_registry.rs"),
...
mod tests {
    ...
    fn extract_into_scans_subdirs() {
        ...
        let registry = std::fs::read_to_string(out.join("rayonette_registry.rs")).unwrap();
    ...
    fn extract_reads_build_env_and_errors_when_unset() {
        ...
        assert_eq!(
            registry,
            "rayonette::agent::Registry::new().with_fn(double)"
        );
    ...
    fn generates_registry_from_functions() {
        ...
        assert_eq!(
            code,
            "rayonette::agent::Registry::new().with_fn(double).with_fn(tasks::score)"
        );
    }
