...
7. trybuild `.stderr` goldens proved too toolchain-fragile and were dropped. CI runs stable and the goldens were generated on nightly, so the first CI run mismatched (the caret span width differs nightly vs stable, and the const-eval panic wording differs even between stable releases, with stable 1.95 not even firing the no-capture const-assert for an unused value). The compile-fail trybuild cases and their goldens were removed entirely. The trybuild pass cases stay (they have no `.stderr` and are robust), and the three failure guarantees (wrong annotation, capturing closure, unrecoverable closure type) are now `compile_fail` doctests on the public surface, which assert only that the code does not compile and therefore hold on every channel.
