...
cleanup() {
  [ "${KEEP:-0}" = 1 ] && return
  docker compose -p rayonette-line2 -f "$TOP/line2/compose.yml" down -t 2 >/dev/null 2>&1
  docker compose -p rayonette-diamond -f "$TOP/diamond/compose.yml" down -t 2 >/dev/null 2>&1
}
trap cleanup EXIT
...
docker compose -p rayonette-line2 -f "$TOP/line2/compose.yml" up -d >/dev/null 2>&1
cfg2=/tmp/rayonette-rr-line2-config
topo_write_config "$cfg2" relay=2210
topo_wait "$cfg2" relay || exit 1
topo_seed_ids rayonette-line2 relay leaf-a leaf-b
topo_children rayonette-line2 relay "$(child leaf-a)" "$(child leaf-b)"
...
docker compose -p rayonette-diamond -f "$TOP/diamond/compose.yml" up -d >/dev/null 2>&1
cfgd=/tmp/rayonette-rr-diamond-config
topo_write_config "$cfgd" relayA=2211 relayB=2212
topo_wait "$cfgd" relayA relayB || exit 1
topo_seed_ids rayonette-diamond relayA relayB leaf
topo_children rayonette-diamond relayA "$(child leaf)"
topo_children rayonette-diamond relayB "$(child leaf)"
