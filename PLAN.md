...
**Delivered.** The coordinator's run loop is elastic: a joins channel feeds handshaked nodes into the live schedule (per-agent vectors grow in lockstep, agents are only appended so indices stay valid), and a rejoin driver retries hosts unreachable at launch on a backoff (bounded attempts, no wall clock) and feeds the ones that come online. The relay gained the same seam: it re-reads its children file on a backoff and splices in newly listed children, advertising the larger capacity up, so the whole tree can grow. Termination moved from channel-close to liveness plus give-up (the coordinator errors only when no agent is alive and no node can still join; a relay tears down on its live-child count reaching zero). Speculative re-execution is opt-in per run via `.speculative()` and off by default: with nothing pending, an idle node re-runs the oldest straggler running on one other agent and races it, the first terminal winning and the rest deduped. A joined node surfaces live with no extra work because the TUI draws straight from the run state, which the join's events populate. The docker bestiarium gained `elastic` (a node absent at launch is started mid-run and absorbed by the rejoin driver) and `relay-grow` (a child appended to a relay's file mid-run is absorbed by its re-read), both validated end to end over real ssh. Carried forward: deduping a late joiner against the running set by id (a joiner is currently treated as a fresh path), a bandwidth probe and per-run metric selector, and deeper-than-depth-2 redundant paths.
...
**Delivered.** The docker `capstone` scenario is segmented, multi-level, redundant, and elastic in one run: two gateways front a shared compute leaf on their own private networks (deduped to one node, one primary and one standby, the leaves named only on the gateways), a distinct compute leaf joins the standby gateway mid-run (the standby re-reads its children file), and the primary gateway is then killed so its shared subtree reroutes onto the standby, with every task completing exactly once. The gateways are relay-only by construction (a relay forwards work and never computes); explicit filter-driven RelayOnly for a compute-capable node is carried forward. The TUI needs nothing new: it draws the run-state tree with single points of failure flagged and a joined node appears in it for free. The real run was executed over Tailscale ssh between physically separate, different-architecture machines: a flat run distributing to a macOS arm64 host as a compute leaf (it probes, builds, and runs every task), and a real multi-level relay tree (Linux coordinator -> macOS gateway -> Linux leaf), each hop reaching the next with its own credentials and the gateway building the agent on the leaf. Both are recorded in the topologies README. This completes the relay-tree epic (R1-R7).
